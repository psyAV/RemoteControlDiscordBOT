...
async def file(ctx, command, *args):
    ...
    Dependencies: filesystem_control, requests, os
    """
    filesystem_control = FileSystemControl(fr"{os.getcwd()}\file")
    ...
    async def save_file(path=None):
        ...
        if r.status_code / 100 != 2:
            raise Exception(f"Download request from Discord returned {r.status_code}")
        file_path = await filesystem_control.save_file(r.content, filename, path)
